...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'element'], function (args) {
        ...
        $.ajax({	//使用JQuery内置的Ajax方法
            ...
            url: "${pageContext.request.contextPath}/Data/getREPTInfor.do",	//请求发送到ShowInfoIndexServlet处
            ...
            success: function (result) {
                ...
                if (result != null && result.length > 0) {
                    for (var i = 0; i < result.length; i++) {
                        <%--if (result[i].hf > ${max_people}) {--%>
                        <%--    layer.msg("警告！当前入园人数超过景区最大客流承载量！")--%>
                        <%--}--%>
                        // console.log(result[i].hf);
                        count.push(result[i].hf);
                        dayDate.push(result[i].day);
                    }
                    myChart.hideLoading();	//隐藏加载动画
                    option.xAxis.data = dayDate;// 设置x轴数据1
                     // option.xAxis[1].data = dayDate;// 设置x轴数据2
                    option.series[0].data = count;// 设置y轴数据1
                    // option.series[1].data = maxPeople;// 设置y轴数据2
